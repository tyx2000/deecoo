...
Operational hardening: tool output (file contents, shell stdout, git diffs, worker prose) is fenced as untrusted data with a nonce-protected delimiter and an injection scan, so the model treats it as data, not instructions. When injection is suspected the raw content is quarantined out-of-band and the model sees only an inert projection with imperative lines withheld. Shell commands that touch credential paths (`~/.ssh`, `~/.aws`, `id_rsa`, `/etc/shadow`, `~/.deecoo/settings.json`, `/proc/*/environ`, `printenv`/`env` dumps) are hard-blocked, secret-looking environment variables are stripped from the child process, and known secret values are redacted from anything a tool surfaces. Network commands that upload local data (`curl -d`, `--post-file`, `nc`) are blocked; set `DEECOO_EGRESS_ALLOWLIST` to a comma list of hosts to additionally block any fetch outside it. A shared rate limiter coordinates the main agent and all workers so parallel dispatch cannot burst the provider. The process guard also detects A→B→A→B oscillation, not just identical repeats. The run footer reports estimated cost.

Determinism: `createReplayClient` / `createReplayTools` replay recorded model and tool results for golden-transcript regression runs with no network or side effects. Concurrent in-process tasks can be isolated with `withIsolatedEnv` / `withIsolatedCwd`, which serialize and restore global `process.env`/cwd.
